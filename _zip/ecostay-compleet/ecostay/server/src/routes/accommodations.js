  ...
  try {
    const [stay] = await query(
      `SELECT id, slug, name, type, city, country, description, website,
              ...
              audit_status, verified_at, created_at, updated_at, canceled_at
...
router.post('/', async (req, res, next) => {
  try {
    const { name, type, city, country, description, website, contact_email } = req.body || {};
    if (!name || !city) {
    ...
    const rows = await query(
      `INSERT INTO accommodations (slug, name, type, city, country, description, website, contact_email)
       VALUES (:slug, :name, :type, :city, :country, :description, :website, :contact_email)
       RETURNING id, slug`,
      {
        slug,
        name,
