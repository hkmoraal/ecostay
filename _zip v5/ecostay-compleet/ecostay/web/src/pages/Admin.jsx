...
import { api, adminKey } from '../api.js';
...
function Stays() {
  ...
  function startEdit(s) {
    setEditing(s.slug);
    setForm({ name: s.name, type: s.type, city: s.city, country: s.country, website: s.website || '', contact_email: s.contact_email || '', description: s.description || '' });
  }
            ...
            {editing === s.slug ? (
              <div className="admin-edit">
                ...
                <div className="btn-row" style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button className="btn" onClick={() => save(s.slug)} disabled={busy}>{busy ? 'Bezig…' : 'Opslaan'}</button>
                </div>
              </div>
            ) : (
              <>
