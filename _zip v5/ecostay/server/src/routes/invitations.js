...
import { sendEmail, invitationEmail, emailConfigured } from '../email.js';
...
router.post('/', requireAdmin, async (req, res, next) => {
  try {
    ...
    // Stuur een uitnodigingsmail (best effort — faalt de mail, dan slaagt de
    // uitnodiging alsnog; de admin ziet de link ook in de UI).
    const certifyUrl = `${FRONTEND_URL}/certificering?invited=${rows[0].slug}`;
    let emailed = false;
    if (contact_email && emailConfigured()) {
      const { subject, html, text } = invitationEmail(name, certifyUrl);
      try {
        await sendEmail({ to: contact_email, subject, html, text });
        emailed = true;
      } catch (mailErr) {
        console.error('[invitations] mail mislukt:', mailErr.message);
      }
    }
