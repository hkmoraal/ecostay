...
import { sendEmail, auditResultEmail, emailConfigured } from '../email.js';
  ...
  try {
    ...
    const rows = await query(
      `SELECT au.id, au.status, au.auditor_name, au.auditor_email,
              au.claimed_at, au.submitted_at, au.created_at, au.kind, au.fee,
...
router.post('/:id/submit', requireAuditor, async (req, res, next) => {
  try {
    ...
    const [audit] = await query(
      'SELECT id, accommodation_id, status, kind FROM audits WHERE id = :id',
      { id: req.params.id }
    );
    if (!audit) return res.status(404).json({ error: 'Audit niet gevonden' });
    ...
    await withTransaction(async (q) => {
      ...
      if (outcome === 'verified') {
        ...
        // Bij een goedgekeurde herbeoordeling: geldigheid met 12 maanden
        // verlengen vanaf de huidige verloopdatum (of vanaf nu als al verlopen).
        if (audit.kind === 'renewal') {
          await q(
            `UPDATE accommodations
                SET expires_at = GREATEST(COALESCE(expires_at, now()), now()) + interval '12 months',
    ...
    // Uitslagmail naar de eigenaar (best effort).
    try {
      const [acc] = await query(
        'SELECT name, slug, contact_email FROM accommodations WHERE id = :id',
        { id: audit.accommodation_id }
      );
      if (acc?.contact_email && emailConfigured()) {
        const { subject, html, text } = auditResultEmail(
          acc.name, outcome === 'verified', `${FRONTEND_URL}/verblijf/${acc.slug}`
        );
        await sendEmail({ to: acc.contact_email, subject, html, text });
      }
    } catch (mailErr) {
      console.error('[audit] uitslagmail mislukt:', mailErr.message);
    }
