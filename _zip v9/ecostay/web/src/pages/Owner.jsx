...
function StayEditor({ stay }) {
  ...
  async function requestRenewal() {
    const price = stay.renewal?.price;
    if (!confirm(`Een herbeoordeling door een auditor aanvragen voor € ${price}? Je certificaat wordt na goedkeuring met een jaar verlengd.`)) return;
    setBusy(true); setError('');
    try {
      await api.ownerRequestRenewal(stay.slug);
      alert('Je herbeoordeling is aangevraagd. Een auditor pikt deze op.');
  ...
  return (
    <div className="owner-stay">
      ...
      {/* Certificaat + herbeoordeling */}
      {stay.status === 'certified' && (
        <>
          <h2 className="section-title" style={{ marginTop: 28 }}>Certificaat</h2>
          <div className="owner-sub">
            <div>
              <div className="muted">
                {stay.expires_at
                  ? <>Geldig tot <strong>{new Date(stay.expires_at).toLocaleDateString('nl-NL')}</strong></>
                  : 'Geldig'}
              </div>
              {stay.renewal?.available && !stay.renewalPending && (
                <div className="muted" style={{ marginTop: 4 }}>
                  {stay.renewal.expired
                    ? 'Je certificaat is verlopen — vraag een herbeoordeling aan.'
                    : `Herbeoordeling nu mogelijk voor € ${stay.renewal.price} (nog ${stay.renewal.monthsUntilExpiry} mnd geldig).`}
                </div>
              )}
              {stay.renewalPending && <div className="muted" style={{ marginTop: 4 }}>Er loopt al een herbeoordeling.</div>}
            </div>
            <div className="owner-sub__actions">
              <a className="btn btn--ghost btn--small" href={`/certificaat/${stay.slug}`} target="_blank" rel="noreferrer">Certificaat bekijken</a>
              {stay.renewal?.available && !stay.renewalPending && (
                <button className="btn btn--small" onClick={requestRenewal} disabled={busy}>
                  Herbeoordeling aanvragen (€ {stay.renewal.price})
                </button>
              )}
            </div>
          </div>
