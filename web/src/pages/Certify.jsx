...
export default function Certify() {
  const [step, setStep] = useState('register'); // register | assess | result
  ...
  const [form, setForm] = useState({
    name: '', type: 'B&B', city: '', country: 'Nederland', website: '', contact_email: '', description: '',
  });

  useEffect(() => {
    if (step !== 'assess') return;
    api.getCriteria().then((d) => setCriteria(d.categories)).catch(() => setError('Criteria laden mislukte.'));
  }, [step]);

  const totalPoints = useMemo(
    () => criteria.reduce((sum, cat) => sum + cat.criteria.reduce((s, c) => s + c.points, 0), 0),
    [criteria]
  );
      ...
      <div className="container container--narrow">
        ...
        {step === 'register' && (
          <section>
            ...
            <p className="muted">
              Vul de basisgegevens in. Daarna doorloop je de zelfscan en zie je meteen welk
              niveau je haalt.
            </p>
            <form className="form" onSubmit={submitRegister}>
              ...
              <div className="field-row">
                <label className="field">
                  ...
                  <select className="input" value={form.type} onChange={(e) => update('type', e.target.value)}>
                    {['B&B', 'Hotel', 'Vakantiehuis', 'Camping', 'Hostel', 'Appartement'].map((t) => (
              ...
              <button className="btn" disabled={busy}>
                {busy ? 'Bezig…' : 'Verder naar de zelfscan'}
              </button>
            </form>
          </section>
        )}
            ...
            <div className="assess">
              <div className="assess__list">
                {criteria.map((cat) => (
                  <fieldset className="assess__group" key={cat.id}>
                    <legend>
                      {cat.name}
                      {cat.intro && <span className="assess__intro"> — {cat.intro}</span>}
                    </legend>
                    {cat.criteria.map((c) => (
              ...
              <aside className="assess__meter">
                <div className="meter-card">
                  ...
                  <button className="btn" onClick={submitAssessment} disabled={busy}>
          ...
          <section className="result">
            <div className={`result__hero result__hero--${result.levelKey}`}>
              ...
              <p className="result__score">
                {result.score}/100 punten ({result.gotPoints} van {result.totalPoints})
              </p>
            </div>
            <p className="muted result__body">
              {result.level > 0
                ? `${stay.name} staat nu in het openbare overzicht met ${result.level} blaadje(s).`
                : `Je hebt minimaal 40% nodig voor brons. Nog ${40 - result.score}% te gaan.`}
            </p>
    ...
    <ol className="steps">
      {items.map((it, i) => (
        <li key={it.key} className={`steps__item ${i <= idx ? 'steps__item--done' : ''} ${i === idx ? 'steps__item--current' : ''}`}>
          <span className="steps__dot">{i + 1}</span>
          {it.label}
        </li>
      ))}
    </ol>
