      ...
      <div className="container">
        ...
        {stay.image_url && (
          <div className="detail__photo">
            <img src={stay.image_url} alt={`Foto van ${stay.name}`} />
          </div>
        )}
        ...
        <header className="detail__head">
          ...
          <aside className={`score-card score-card--${stay.levelKey}${["invited","draft","canceled"].includes(stay.status) ? " score-card--neutral" : ""}`}>
            ...
            {stay.audit_status === 'verified' && (
              <span className="verified-badge" title={stay.verified_at ? `Geverifieerd op ${new Date(stay.verified_at).toLocaleDateString('nl-NL')}` : 'Geverifieerd door auditor'}>
                ✓ Geverifieerd door auditor
              </span>
            )}
          </aside>
        </header>
